...
// Admin LINE binding endpoints. Lets a Google-authenticated admin link a
// LINE userId so they receive /api/line/broadcast messages alongside guild
// members.
//
// Schema:
...
module.exports = function registerAdminBindRoutes(app, firebase, requireAdmin) {
    ...
    app.get('/api/admin/line-bind', requireAdmin, async (req, res) => {
        try {
            ...
            if (!email) return res.status(400).json({ ok: false, error: '缺少 admin email' });
            const db = firebase.getDb();
            if (!db) return res.status(503).json({ ok: false, error: 'Firestore 連線失敗' });
            const snap = await db.collection('adminLineBinds').doc(email).get();
            if (!snap.exists) return res.json({ ok: true, bound: false, email });
    ...
    app.delete('/api/admin/line-bind', requireAdmin, async (req, res) => {
        try {
            ...
            const snap = await db.collection('adminLineBinds').doc(email).get();
            if (!snap.exists) return res.json({ ok: true, alreadyUnbound: true });

            await db.collection('adminLineBinds').doc(email).delete();
            logActivity(firebase, {
                action: 'unbind',
                module: 'admin',
                actor: email,

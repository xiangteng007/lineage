...
module.exports = function registerAuthRoutes(app, firebase, agg) {
    app.get('/api/me', async (req, res) => {
        try {
            ...
            const token = req.headers['x-auth-token'] || req.headers['x-firebase-token'];
            if (token) {
                try {
                    const decoded = authTokens.verify(token);
                    lineUserId = decoded.uid || lineUserId; // token uid === lineUserId
                } catch (e) {
